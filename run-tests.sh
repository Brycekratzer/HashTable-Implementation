...
do
    ...
    dos2unix word-list-$load-linear-dump.txt word-list-$load-double-dump.txt  >& /dev/null

    echo
    diff -w -B  word-list-$load-linear-dump.txt test-cases/word-list-$load-linear-dump.txt > diff-linear-$load.out
    if test "$?" = 0
    then
    ...
    diff word-list-$load-double-dump.txt test-cases/word-list-$load-double-dump.txt > diff-double-$load.out
    if test "$?" = 0
    then
